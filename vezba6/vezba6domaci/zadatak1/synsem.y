...
calculate_statement
  : _CALCULATE _ID _LPAREN rel_exp _RPAREN _TRUE _COLON num_exp _SEMICOLON _FALSE _COLON num_exp _SEMICOLON 
	 {
        int idx = lookup_symbol($2, (VAR|PAR));
        if(idx == -1)
          err("invalid lvalue '%s' in assignment", $2);
        else
          if(get_type(idx) != get_type($8) && get_type(idx) != get_type($12))
